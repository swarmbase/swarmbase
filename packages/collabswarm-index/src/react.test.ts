...
// We test the hooks' logic by directly exercising subscribe() and defineIndex(),
// since jsdom + React rendering would require heavier test infrastructure.
...
describe('React hook backing logic (subscribe + defineIndex)', () => {
  ...
  test('subscribe delivers initial empty result', async () => {
  ...
  test('unsubscribe prevents further callbacks', async () => {
    const results: QueryResult<Record<string, unknown>>[] = [];
    const unsub = manager.subscribe(
      { indexName: 'items', filters: [] },
      (r) => results.push(r),
  ...
  test('defineIndex + removeIndex lifecycle', async () => {
    const newDef = {
      name: 'tags',
