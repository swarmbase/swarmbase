...
Current CI verifies relay builds and unit tests. Peer discovery, GossipSub, WebRTC, Circuit Relay, DCUtR/STUN/TURN configuration, bootstrap, and DHT are partial. A dedicated clean-topology test verifies initial encrypted Automerge document load across NAT through a relay. Live post-load pubsub convergence, partition/rejoin, relay failover during edits, TURN-authenticated behavior, transport-specific WebSocket/WebTransport sync, and larger multi-peer churn are not established. See [Limitations](../limitations/).
